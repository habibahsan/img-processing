...
def main():
    ...
    for item in file_list.values():
        if item[0]['base'] == item[1]['base']:
            i1 = Image.open(os.path.abspath(
                os.path.join(dirname, item[0]['loc'])))
            i2 = Image.open(os.path.abspath(
                os.path.join(dirname, item[1]['loc'])))
            ov = os.path.join(
                dirname, 'out', (item[0]['base'] + '_v' + item[0]['ext']))
            oh = os.path.join(
                dirname, 'out', (item[0]['base'] + '_h' + item[0]['ext']))
